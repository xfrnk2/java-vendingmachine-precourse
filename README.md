...
### 자판기 
1. [x] 자판기 보유 금액을 최소한의 동전으로 반환하는 기능
2. [x] 입력 값을 상품별로 분리하고 상품명,가격,수량 단위로 상품을 저장하는 기능
3. [x] 2의 상품의 개별 가격이 100원 이상이고 10으로 나누어지는지 검증하는 기능
4. [x] 금액을 투입하여 상품을 구매하는 기능
    - [x] 모든 상품 소진 또는 남은 금액이 상품 최저가보다 작을 시 구매를 계속 할 수 없다
5. [x] 잔돈을 반환하는 기능
...
### 출력
1. [x] 자판기가 보유한 동전 출력 기능
2. [x] 잔돈 출력 기능

## 예외 처리

1. 입력받은 자판기 보유 금액이 숫자가 아닌 경우
2. 등록할 상품 가격이 100원 미만이거나 10으로 나누어떨어지지 않는 경우
3. 상품 목록을 입력 받을 때 빈 값인 경우
4. 상품 목록을 입력 받을 때 세미콜론만 있는 경우
5. 입력받은 상품 목록을 세미콜론으로 구분했을 때 하나의 상품 상세가 대괄호로 시작해서 대괄호로 끝나지 않는 경우
6. 개별 상품 정보를 쉼표로구분했을 때 총 3개의 값으로 나뉠수 없는 경우
7. 상품명, 가격, 수량 중 어느 하나라도 빈 값이 존재하는 경우
8. 상품명이 숫자인 경우
9. 가격이 문자이거나 음의 정수인 경우
10. 수량이 문자이거나 음의 정수인 경우
11. 투입한 금액이 빈 값인 경우
12. 입력받은 투입한 금액이 문자이거나 음의 정수인 경우
13. 입력받은 구매할 상품명이 빈 값이거나 숫자인 경우
